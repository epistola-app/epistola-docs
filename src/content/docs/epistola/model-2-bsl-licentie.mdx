...
description: Het antwoord op de kernvraag wanneer externe borging ontbreekt — de licentie borgt de bijdrage zelf, en maakt een markt van concurrerende leveranciers mogelijk
---
...
De kernvraag is: *kan de bijdrage aan het onderhoud worden geborgd?* Model 1 is het antwoord als die borging ergens buiten de software bestaat. Model 2 is het antwoord wanneer die externe borging **ontbreekt** — dan moet het borgingsmechanisme in de software zelf zitten.

Want zonder borging beloont een diensten-only model structureel het verkeerde gedrag: Epistola BV heeft geen reden om een ecosysteem te bouwen omdat elk nieuw ecosysteem-lid haar omzet bedreigt.

Model 2 lost dat op door de bijdrage in de licentie te borgen en de inkomstenstroom te splitsen. De stichting verdient op licenties. De BV verdient op diensten — maar nu als één van meerdere leveranciers in een open markt.
...
<CardGrid>
  <Card title="In Model 1: ecosysteemgroei schaadt de omzet" icon="warning">
    Elke supplier die diensten aanbiedt, neemt omzet weg bij Epistola BV. Het ecosysteem laten groeien betekent je eigen inkomsten ondermijnen — dus gebeurt het niet.
  </Card>
  <Card title="In Model 2: ecosysteemgroei vergroot de omzet" icon="approve-check-circle">
    Elke nieuwe preferred supplier betaalt licentieafdracht aan de stichting en brengt nieuwe gemeenten binnen. Meer suppliers = meer licenties = beter gefinancierd platform — ook voor de BV.
  </Card>
</CardGrid>
...
```mermaid
flowchart TB
    S["🏛️ Stichting<br/>IP, licentie, governance"]
    BV["🏢 Epistola BV<br/>Platform + kernonderhoud + diensten"]
    A["🏢 Preferred Supplier A<br/>SaaS hosting"]
    B["🏢 Preferred Supplier B<br/>Implementatie & migratie"]
    C["🏢 Preferred Supplier C<br/>Template QA + training"]
    G["🏘️ Gemeenten"]
...
In Model 1 levert Epistola BV zelf (vrijwel) alle diensten. Hoe meer gemeenten meedoen, hoe meer dienstverlening nodig is — en dus hoe groter de BV zou moeten worden. Maar de BV mag statutair niet boven de 20 FTE groeien. Die vaste grens wordt dan het plafond: juist als het platform succesvol wordt, loopt de groei vast op de enige partij die de diensten levert.
...
- [Model 1: Open Source + Diensten](/epistola/model-1-open-source) — Het andere antwoord: borging buiten de licentie
- [Organisatiestructuur](/epistola/organisatiestructuur) — De BV-stichting structuur die beide antwoorden draagt
