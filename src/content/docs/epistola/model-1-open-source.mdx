...
Voor Epistola — met de ambitie om alle 300+ Nederlandse gemeenten te bedienen en een gezond ecosysteem van leveranciers te bouwen — is Model 1 **zonder geborgde bijdrage** structureel ongeschikt. Niet omdat het onethisch is, maar omdat de incentives verkeerd liggen. De eigenaarschapskant is bij Epistola al geregeld via de stichting-BV-structuur; daardoor is de externe borging de enige nog ontbrekende voorwaarde. **Mét** die borging is Model 1 juist een uitstekend antwoord. Zie [de kernvraag](/epistola/modelkeuze) voor hoe dit zich verhoudt tot Model 2.
